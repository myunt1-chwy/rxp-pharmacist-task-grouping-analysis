#!/usr/bin/env python3
"""Create the task-sequence table in Snowflake."""
...
TARGET_TABLE = "EDLDB_DEV.PET_HEALTH_ANALYTICS_SANDBOX.MY_RXP_TASKS_WITH_SEQUENCE"
REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
TEMPLATE_DIRECTORY = Path(__file__).resolve().parents[1] / "sql"
TEMPLATE_NAME = "create_task_sequence_table.sql.j2"
GENERATED_SQL_PATH = REPOSITORY_ROOT / "generated" / "generate_task_sequence_table.sql"
...
def main(env_file: Path, dry_run: bool) -> None:
    """Build MY_RXP_TASKS_WITH_SEQUENCE from MY_RXP_TASKS."""
    sql = render_sql()
    try:
